...
stdex::math::matrix<_Tp> stdex::math::matrix<_Tp>::operator *(const stdex::math::matrix<_Tp>& other) const {
	...
	for (int i=0;i<x_dimension_;i++) {
		for (int j=0;j<other.y_dimension_;j++) {
			...
			for (int k=0;k<y_dimension_;k++) {
				temp_value=temp_value+m_[i][k]*other.m_[k][j];
			}
...
_Tp& stdex::math::matrix<_Tp>::operator ()(size_t i,size_t j) {
	if (i>=x_dimension_ || j>=y_dimension_) {
...
stdex::math::matrix<_Tp> stdex::math::matrix<_Tp>::adjoint() const {
	...
	for (int i=0;i<x_dimension_;i++) {
		for (int j=0;j<y_dimension_;j++) {
			result.m_[j][i]=expansion(i,j).value()*((i+j)%2?-1:1);
		}
	}
	return result;
}
...
stdex::math::matrix<_Tp> stdex::math::matrix<_Tp>::inverse() const {
	...
	for (int i=0;i<dimension;i++) {
		for (int j=0;j<dimension;j++) {
			if (i==j) {
				temp.m_[i][j+dimension]=stdex::math::base_unit_trait<_Tp>::value();
			} else {
				temp.m_[i][j+dimension]=stdex::math::base_unit_trait<_Tp>::zero();
			}
		}
	}
    	for (int i=0;i<dimension;i++) {
	...
	for (int i=0;i<dimension;i++) {
		for (int j=0;j<dimension;j++) {
			result.m_[i][j]=temp.m_[i][j+dimension];
		}
	}
	return result;	
}
...
stdex::math::matrix<_Tp> stdex::math::matrix<_Tp>::power(int times) const {
	...
	if (times==0) {
		for (int i=0;i<dimension;i++) {
			for (int j=0;j<dimension;j++) {
				if (i==j) {
					result.m_[i][j]=stdex::math::base_unit_trait<_Tp>::value();
				} else {
					result.m_[i][j]=stdex::math::base_unit_trait<_Tp>::zero();
				}
			}
		}
		return result;
...
stdex::math::matrix<_Tp> stdex::math::matrix<_Tp>::expand_to(size_t to_x,size_t to_y,size_t x_dimension,size_t y_dimension) const {
	if (x_dimension<x_dimension_ || y_dimension<y_dimension_ || (x_dimension==x_dimension_ && y_dimension==y_dimension_)) {
		throw std::invalid_argument("Target dimensions are too small");
	}
	...
	for (int i=0;i<x_dimension_;i++) {
		for (int j=0;j<y_dimension_;j++) {
			result.m_[i+to_x][j+to_y]=m_[i][j];
		}
	}
	return result;
}
...
stdex::math::matrix<_Tp> stdex::math::matrix<_Tp>::crop_to(size_t from_x,size_t from_y,size_t x_dimension,size_t y_dimension) const {
	if (x_dimension>x_dimension_ || y_dimension>y_dimension_ || (x_dimension==x_dimension_ && y_dimension==y_dimension_)) {
		throw std::invalid_argument("Target dimensions are too large");
	}
	...
	for (int i=0;i<x_dimension;i++) {
		for (int j=0;j<y_dimension;j++) {
			result.m_[i][j]=m_[i+from_x][j+from_y];
		}
	}
	return result;
}
...
_Tp stdex::math::matrix<_Tp>::value() const {
	...
	for (int i=0;i<dimension;i++) {
		...
		for (int j=i+1;j<dimension;j++) {
			if (abs(m[j][i])>abs(m_[pivot][i])) {
				pivot=j;
			}
		}
		if (m_[pivot][i]==0) {
			return stdex::math::base_unit_trait<_Tp>::zero();
		}
		if (i!=pivot) {
			_Tp* temp_row=m_[i];
			m_[i]=m_[pivot];
...
std::string stdex::math::matrix<_Tp>::print() const {
	...
	for (int j=0;j<y_dimension_;j++) {
		int curr_width=0;
		for (int i=0;i<x_dimension_;i++) {
			curr_width=std::max(curr_width,(int)stdex::math::base_unit_trait<_Tp>::to_string(m_[i][j]).size());
		}
		curr_width+=2;
