...
public:
	...
	using boolean_t=typename _Json::boolean_t;
	using string_t=typename _Json::string_t;
	using node_t=typename _Json::base_t;
...
private:
	...
	int parse_not(std::size_t& pos,bool negated=false) {
		skip_blank(pos);
		if (pos<expression_.size() && expression_[pos]=='!') {
			pos++;
			const int child=parse_not(pos,true);
		...
		return parse_basic(pos,negated);
	}
	...
	int parse_basic(std::size_t& pos,bool negated=false) {
		skip_blank(pos);
		...
		if (has_comparison) {
			if (negated) fail(operand_position,"'!' cannot be applied to a comparison, parentheses are required");
...
private:
	...
	static bool decode_string(const char* first,const char* last,string_t& out,std::string& error_message) {
		...
		while (first<last) {
			if (*first!='\\') {
				...
				for (std::size_t i=0;i<=extra;i++) out.push_back(first[i]);
				first+=extra+1;
				continue;
			}
			first++;
	...
	//The string production either matches as a whole or not at all, so a failed
	//match is re-scanned here to point at the byte that actually broke it.
	static void diagnose_string(const char* first,const char* const last,std::size_t base,std::size_t& error_position,std::string& error_message) {
		const char* it=first+1;
		while (it<last) {
			const unsigned char c=static_cast<unsigned char>(*it);
			if (c=='"') {
				error_position=base;
				error_message="Invalid string literal";
				return;
			}
			if (c<0x20) {
				error_position=base+static_cast<std::size_t>(it-first);
				error_message="Control character in string";
				return;
			}
			if (c!='\\') {
				it++;
			...
			for (int i=0;i<4;i++) {
				if (it>=last) {
					error_position=base+static_cast<std::size_t>(it-first);
					error_message="Truncated \\u escape";
					return;
				}
				if (!is_hexadecimal_digit(*it)) {
					error_position=base+static_cast<std::size_t>(it-first);
					error_message="Invalid hexadecimal digit in \\u escape";
					return;
				}
				it++;
			}
	...
	static parser_t& grammar() {
		static thread_local parser_t instance(JS_START,JS_EPSILON,JS_EOF);
...
public:
	//Children of a json are plain dom nodes, so serialising a subtree through
	//the member dump() would require a deep copy; this entry point does not.
