...
//与binary_data_type/xml_data_type等其他并行派生kind数值重叠(均从dom_data_type末尾续号),
//不同记法的扩展节点不能混入同一棵树;跨记法转换请走convert_to/assign_converted协议。
_STDEX_DERIVED_KIND(toml_data_type,structure::dom_data_type,_STDEX_KIND_AUTO_START,
	_STDEX_KIND_VALUE_AUTO(TDT_DATETIME)
)

//TOML的四种RFC 3339日期时间变体。
...
//TOML独有标量:日期时间。四种变体共用一个载荷结构,variant指明有效字段。
//offset_minutes仅TDV_OFFSET_DATETIME有效,UTC(Z)为0,东八区为480,西五区为-300。
...
//终结符与非终结符。词法是双模式的(KEY/VALUE),表头方括号与值数组方括号
//在词法层即被区分为不同终结符,使SLR表无需依赖上下文消歧。
...
enum toml_production : int {
	TP_START,//START→TOML EOF
	TP_TOML,//TOML→LINE_SEQ
	TP_LINES_FIRST,//LINE_SEQ→LINE
	TP_LINES_APPEND,//LINE_SEQ→LINE_SEQ LINE
	TP_LINE_BLANK,//LINE→NEWLINE
	TP_LINE_KEYVAL,//LINE→KEYVAL NEWLINE
	TP_LINE_TABLE,//LINE→TABLE NEWLINE
	TP_LINE_ARRAY_TABLE,//LINE→ARRAY_TABLE NEWLINE
	TP_TABLE,//TABLE→TABLE_OPEN KEYPATH TABLE_CLOSE
	TP_ARRAY_TABLE,//ARRAY_TABLE→ARRAY_TABLE_OPEN KEYPATH ARRAY_TABLE_CLOSE
	TP_KEYVAL,//KEYVAL→KEYPATH EQ VALUE
	TP_KEYPATH_FIRST,//KEYPATH→KEY
	TP_KEYPATH_APPEND,//KEYPATH→KEYPATH DOT KEY
	TP_VALUE_STRING,//VALUE→STRING
	TP_VALUE_INT,//VALUE→INT
	TP_VALUE_FLOAT,//VALUE→FLOAT
	TP_VALUE_BOOL,//VALUE→BOOL
	TP_VALUE_DATETIME,//VALUE→DATETIME
	TP_VALUE_ARRAY,//VALUE→ARRAY
	TP_VALUE_INLINE_TABLE,//VALUE→INLINE_TABLE
	TP_ARRAY_EMPTY,//ARRAY→[ ]
	TP_ARRAY,//ARRAY→[ ELEMENTS ]
	TP_ARRAY_TRAILING,//ARRAY→[ ELEMENTS , ]
	TP_ELEMENTS_FIRST,//ELEMENTS→VALUE
	TP_ELEMENTS_APPEND,//ELEMENTS→ELEMENTS COMMA VALUE
	TP_INLINE_TABLE_EMPTY,//INLINE_TABLE→{ }
	TP_INLINE_TABLE,//INLINE_TABLE→{ MEMBERS }
	TP_MEMBERS_FIRST,//MEMBERS→KEYVAL
	TP_MEMBERS_APPEND,//MEMBERS→MEMBERS COMMA KEYVAL
};

//TOML的SAX事件接口。与json_sax不同,TOML是行导向+显式路径的格式:
//table/array_table对应[a.b]与[[a.b]]表头(path为点分段序列,自文档根解释);
//key在每个键值对的值之前到达(path自当前表/当前内联表解释);
//值事件(string/number/boolean/datetime/容器)紧随其后。
//注意:键重复、表重定义等语义合法性由树构建方(builder)裁决,
//acceptor路径上的accept()只验证词法与文法。
...
//SAX→DOM构建器。除装树外还实现TOML的全部表语义规则:
//键不可重复定义;[t]表头不可重复声明;点分键创建的表不可被表头重开或扩展;
//内联表与值数组封闭后不可扩展或追加;[[t]]只能追加到自身创建的表数组。
//节点身份用影子树(路径同构的元数据树)而非节点地址记录,以免表数组扩容搬移失效。
template <typename _Toml>
...
private:
	...
	//表头导航:[path]与[[path]]均自根解释。中间段:缺失则隐式建表;
	//已存在则必须是可经过的表(隐式/显式表头/表数组的当前末元素),
	//点分键建的表与封闭节点均不可经过。
	bool navigate_header(const path_t& path,bool as_array_table) {
		...
		if (!as_array_table) {
			...
			csh->children.clear();//新元素=全新作用域
		}
		current_shadow_=csh;
		return true;
	}
	//点分键值对落点解析:在当前表上下文中带影子规则;在打开的内联表中
	//只需局部规则(内联表诞生时为空,其内部对象子节点只能源自本内联表的点分键,
	//唯一需要排除的是已作为完整内联表值封闭的子节点)。
...
protected:
	//TOML独有节点的载荷子类。clone/destroy/destroy_self遵循dom的载荷协议
	//(虚函数仅在派生根对象生效;子节点切片为dom后由克隆出的载荷自描述地维护自身)。
...
private:
	...
	//单行basic字符串解码(输入含首尾引号,正则已保证形态合法,仅需校验\u/\U码点)。
	...
	//多行basic字符串扫描(first指向开头三引号之后)。规则:紧随开界符的首个换行剔除;
	//行尾反斜杠(其后至行尾仅空白)吞掉直到下一个非空白的全部空白与换行;
	//一至两个连续引号是内容,三引号收界(收界前可再多带至多两个引号作为内容)。
	...
	static long fraction_to_nanoseconds(const char* first,const char* last) noexcept {
		...
		for (;digits<9;digits++) result*=10;//不足9位右补零
		return result;//超过9位按规范截断
	}
	...
	//双模式上下文词法。mode_key=true时按键位置取词(裸键/引号键/表头括号/点/等号),
	//否则按值位置取词(标量/数组/内联表括号)。ctx栈记录括号上下文:
	//'H'=[表头],'D'=[[表数组头]],'A'=值数组,'T'=内联表。
	//换行语义:顶层是终结符;数组内被抑制;内联表与表头内非法。
		...
		while (first<last) {
			...
			if (mode_key) {
				...
				if (c=='[') {
					...
					//TOML语义:整数必须无损表示,溢出是错误而非静默降级为浮点(与json不同)。
					if (errno==ERANGE || value<static_cast<long long>((std::numeric_limits<int_t>::min)()) || value>static_cast<long long>((std::numeric_limits<int_t>::max)())) {
						error_message="integer does not fit into int_t";
						return false;
					}
	...
	//移进/归约监听器→SAX事件适配。键路径在TP_KEYPATH_*归约时增量累积;
	//key事件在移进'='时分发(先于值事件,保证构建方顺序);表头事件在收界归约时分发。
	//下标约定:on_shift的tokens_[id-1]=当前移进词;on_reduction的tokens_[id-2]=最后被消耗的终结符。
...
public:
	static bool sax_parse(std::string_view input,sax_t* sax) {
		...
		//与json不同:空(或仅注释/空白)的TOML文档是合法的空表,直接成功返回,
		//不产生任何事件(builder在构造时已把根初始化为对象)。
...
private:
	...
	//basic字符串转义。TOML允许制表符原样出现;其余C0控制字符与DEL转义;
	//ensure_ascii时按UTF-8解码非ASCII序列,BMP内写\u,其外写\U(TOML有\U,无需代理对)。
	...
	//TOML原生支持inf/nan(与json落null不同);普通值要求带小数点或指数。
	...
	//表数组判定:非空且全为对象的数组以[[...]]小节形式输出;
	//其余数组(含空数组与混合数组)以内联形式输出,其中的对象渲染为内联表。
	...
	//两遍输出:先简单键值行,再[子表]与[[表数组]]小节(每个小节前隔一空行)。
	//indent>0时小节标题缩进depth级,小节内键值行缩进depth+1级(TOML缩进不影响语义)。
...
public:
	//序列化。TOML文档的顶层必须是表;indent<=0为常规无缩进形态,
	//indent>0时子表小节按层级缩进(仅排版,无语义)。ensure_ascii转义全部非ASCII。
