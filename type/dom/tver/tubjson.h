...
//@Version 1.0.0.0
//ubjson:dom的UBJSON(Draft 12)具体记法(继承structure::binary_dom)。读写经bitwise::bit_reader/bit_writer,
//字节序BO_MSBYTE(UBJSON规定多字节量为大端)。
//类型映射:Z→null;T/F→bool;i/U/I/l/L→整数(超出int_t值域抛);d/D→浮点;S→字符串;C(char)按规范
//"等价于长度1的字符串"落为string(规范原文背书,编码恒以S发出);H(高精度数)为非dom原生类型,
//新增派生kind UBDT_HIGH_PRECISION(串载荷)完整承载;N(no-op)为填充而非值,解码一律跳过。
//容器:支持强类型/计数优化的完整解码——[或{后可选$类型与#计数($出现则#必现,元素省略类型标记;
//$Z/$T/$F配合计数为零载荷重复);对象键恒为无S前缀的长度串。编码恒产朴素容器(逐元素带标记),
//唯BDT_BINARY按惯例编码为[$U#强类型字节数组;对应地,一维$U#强类型数组解码回BDT_BINARY,
//该映射双向自洽(UBJSON无二进制类型,uint8强类型数组即其字节序列惯用形)。
//长度/计数:任意整数标记(i/U/I/l/L),负值抛错;重复键保留先出现者;编码遇未知派生kind抛
...
//UBJSON专有kind:自binary_data_type(BDT_BINARY)之后续号。
...
public:
	...
	//支持集=binary_dom支持集+UBDT_HIGH_PRECISION。
...
private:
	...
	static dom_t decode_array(bitwise::bit_reader& reader) {
		...
		if (typed && element_type=='U') {//一维uint8强类型数组:即UBJSON的字节序列惯用形,对应BDT_BINARY
			dom_t node;
