...
//@Version 1.0.0.0
//bjdata:dom的BJData(Draft 2/3)具体记法(继承structure::binary_dom)。读写经bitwise::bit_reader/bit_writer,
//字节序BO_LSBYTE(BJData与UBJSON的关键差异之一:规定多字节量为小端)。
//类型映射:Z→null;T/F→bool;i/U/I/u/l/m/L/M/B→整数(u/m/M为BJData新增的uint16/32/64,B为字节;
//超出int_t值域抛);h(float16)按半精度解码、d/D→浮点(编码恒D);S→字符串;C按规范"等价于长度1的
//字符串"落为string;H(高精度数)为非dom原生类型,新增派生kind BJDT_HIGH_PRECISION(串载荷)完整承载;
//N(no-op)为填充而非值,解码一律跳过。
//容器:支持强类型/计数优化的完整解码($出现则#必现);BJData新增ND数组——#后跟维度数组
//([dim0,dim1,...],维度数组本身复用通用解码故亦可为优化形),元素按行主序读取并展开为嵌套数组
//(ND是编码优化而非数据类型,不新设kind);对象键恒为无S前缀的长度串。编码恒产朴素容器,
//唯BDT_BINARY用BJData原生字节类型编码为[$B#强类型数组;对应地,一维计数形$B#解码回BDT_BINARY
//(ND形与$U#保持整数数组,B才是BJData的字节语义)。
//长度/计数:任意整数标记,负值抛错;重复键保留先出现者;编码遇未知派生kind抛std::invalid_argument。
#ifndef _STDEX_TYPE_DOM_BJDATA_H_
#define _STDEX_TYPE_DOM_BJDATA_H_ 1
...
namespace basic_bjdata {

//BJData专有kind:自binary_data_type(BDT_BINARY)之后续号。
_STDEX_DERIVED_KIND(bjdata_data_type,structure::binary_data_type,_STDEX_KIND_AUTO_START,
	_STDEX_KIND_VALUE_AUTO(BJDT_HIGH_PRECISION)
)

_STDEX_DOM_TPL_DECLARATION
class bjdata : public structure::binary_dom<_Int,_Float,_Boolean,_String,_Array,_Object,_Allocator> {
public:
...
protected:
	...
	struct high_precision_value : base_t::value_t {
		...
		typename base_t::value_t* clone(structure::dom_data_type t) const override {
			if (t==BJDT_HIGH_PRECISION) return create_value<high_precision_value>(*this);
			return base_t::value_t::clone(t);
		}
		void destroy(structure::dom_data_type t) override {
			if (t==BJDT_HIGH_PRECISION) return;
	...
	static high_precision_value* high_precision_payload(const base_t& node) {
		...
		if (node.type()!=bjdata_data_type(BJDT_HIGH_PRECISION) || !payload) throw std::invalid_argument("Node does not hold a bjdata high-precision payload");
		return payload;
	}
...
public:
	...
	bjdata()=default;
	~bjdata() override=default;

	bjdata(const bjdata&)=default;
	bjdata(bjdata&&) noexcept=default;

	bjdata& operator =(const bjdata&)=default;
	bjdata& operator =(bjdata&&)=default;

	//支持集=binary_dom支持集+BJDT_HIGH_PRECISION。
	bool support(structure::dom_data_type t) const noexcept override {
		return base_t::support(t) || t==BJDT_HIGH_PRECISION;
	}

	//---工厂---
	static base_t make_high_precision(string_t literal) {
		base_t node;
		node.data()=typename base_t::data_t(structure::dom_data_type(BJDT_HIGH_PRECISION),create_value<high_precision_value>(std::move(literal)));
		return node;
	}
	static bjdata binary(binary_t data) {
		bjdata result;
		result.set_binary(std::move(data));
		return result;
	}
	static bjdata high_precision(string_t literal) {
		return bjdata(make_high_precision(std::move(literal)));
	}

	//---判断---
	static bool is_high_precision(const base_t& node) noexcept {
		return node.type()==bjdata_data_type(BJDT_HIGH_PRECISION);
	}
	bool is_high_precision() const noexcept {
		return is_high_precision(*this);
	}

	//---访问(工厂high_precision对const char*实参与访问器会双用户转换歧义,访问器按content先例命名)---
	...
	//---解码---
	static bjdata parse(const std::uint8_t* data,std::size_t size) {
		bitwise::bit_reader reader(data,size,bitwise::BO_LSBYTE);
		bjdata result(decode_value(reader));
		skip_no_op(reader);
		if (!reader.eof()) throw std::runtime_error("Trailing bytes after bjdata value at byte "+std::to_string(base_t::byte_position(reader)));
		return result;
	}
	static bjdata parse(const binary_t& data) {
...
protected:
	//记法转换协议·源侧降级:先交由binary_dom处理BDT_BINARY;BJDT_HIGH_PRECISION降级为字符串
	...
	bool degrade_unsupported(const base_t& source,base_t& replacement) const override {
		...
		if (source.type()!=bjdata_data_type(BJDT_HIGH_PRECISION)) return false;
...
private:
	...
	static bool is_integer_marker(std::uint8_t marker) noexcept {
		return marker=='i' || marker=='U' || marker=='I' || marker=='u' || marker=='l' || marker=='m' || marker=='L' || marker=='M' || marker=='B';
	}
	static bool is_value_marker(std::uint8_t marker) noexcept {
		return marker=='Z' || marker=='T' || marker=='F' || marker=='C' || marker=='S' || marker=='H' || marker=='h' || marker=='d' || marker=='D' || is_integer_marker(marker);
	}
	static std::int64_t read_integer(std::uint8_t marker,bitwise::bit_reader& reader,std::size_t start) {
		switch (marker) {
			...
			case 'U':
			case 'B': return reader.read_u8();
			case 'I': return static_cast<std::int16_t>(reader.read_u16());
			case 'u': return reader.read_u16();
			case 'l': return static_cast<std::int32_t>(reader.read_u32());
			case 'm': return reader.read_u32();
			case 'L': return static_cast<std::int64_t>(reader.read_u64());
			case 'M': {
				const std::uint64_t value=reader.read_u64();
				if (value>static_cast<std::uint64_t>((std::numeric_limits<std::int64_t>::max)())) throw std::runtime_error("Unsigned integer does not fit into int_t at byte "+std::to_string(start));
	...
	static dom_t decode_with_marker(std::uint8_t marker,bitwise::bit_reader& reader) {
		...
		switch (marker) {
			...
			case 'm':
			case 'L':
			case 'M': return decode_integer(marker,reader,start);
			case 'h': return dom_t(static_cast<float_t>(decode_half(reader.read_u16())));
			...
			default: throw std::runtime_error("Invalid bjdata type marker at byte "+std::to_string(start));
		}
	}
	...
	//容器优化头:[或{之后的可选$类型与#;#后为计数,或[开头的维度数组(ND形,置dimensions)。
	static void decode_container_head(bitwise::bit_reader& reader,bool& typed,std::uint8_t& element_type,bool& counted,std::size_t& count,std::vector<std::size_t>& dimensions) {
		typed=false;
		counted=false;
		element_type=0;
		count=0;
		...
		if (!reader.eof() && reader.peek_bits<std::uint8_t>(8)=='$') {
			...
			read_count_or_dimensions(reader,counted,count,dimensions);
			return;
		}
		if (!reader.eof() && reader.peek_bits<std::uint8_t>(8)=='#') {
			reader.read_u8();
	...
	static void read_count_or_dimensions(bitwise::bit_reader& reader,bool& counted,std::size_t& count,std::vector<std::size_t>& dimensions) {
		counted=true;
		if (!reader.eof() && reader.peek_bits<std::uint8_t>(8)=='[') {//ND形:维度数组(自身复用通用解码,可为优化形)
			const std::size_t start=base_t::byte_position(reader);
			dom_t dims=decode_value(reader);
			if (!dims.is_array() || dims.empty()) throw std::runtime_error("Invalid dimension array at byte "+std::to_string(start));
			std::size_t total=1;
			for (auto it=dims.cbegin();it!=dims.cend();it++) {
				if ((*it).type()!=structure::DDT_INT || (*it).value().integer<0) throw std::runtime_error("Invalid dimension array at byte "+std::to_string(start));
				const std::size_t dimension=static_cast<std::size_t>((*it).value().integer);
				if (dimension!=0 && total>(std::numeric_limits<std::size_t>::max)()/dimension) throw std::runtime_error("Dimension product overflow at byte "+std::to_string(start));
				total*=dimension;
	...
	static dom_t build_dimensional(const std::vector<std::size_t>& dimensions,std::size_t level,std::vector<dom_t>& flat,std::size_t& cursor) {
		dom_t result(structure::DDT_ARRAY);
		typename dom_t::array_t& array=*result.value().array;
		if (level+1==dimensions.size()) {
			for (std::size_t i=0;i<dimensions[level];i++) array.push_back(std::move(flat[cursor++]));
			return result;
		}
		for (std::size_t i=0;i<dimensions[level];i++) array.push_back(build_dimensional(dimensions,level+1,flat,cursor));
		return result;
	}
	static dom_t decode_array(bitwise::bit_reader& reader) {
		...
		decode_container_head(reader,typed,element_type,counted,count,dimensions);
		if (typed && element_type=='B' && dimensions.empty()) {//一维字节强类型数组:BJData的原生字节语义,对应BDT_BINARY
			dom_t node;
			base_t::set_binary(node,base_t::template read_block<binary_t>(reader,count));
			return node;
		}
		if (!dimensions.empty()) {//ND形:行主序读平,再按维度展开为嵌套数组
			std::vector<dom_t> flat;
			flat.reserve(count);
			for (std::size_t i=0;i<count;i++) {
				if (typed) flat.push_back(decode_with_marker(element_type,reader));
				else flat.push_back(decode_value(reader));
			}
			std::size_t cursor=0;
			return build_dimensional(dimensions,0,flat,cursor);
		}
		dom_t result(structure::DDT_ARRAY);
		typename dom_t::array_t& array=*result.value().array;
		if (counted) {
	...
	static dom_t decode_object(bitwise::bit_reader& reader) {
		...
		std::vector<std::size_t> dimensions;
		decode_container_head(reader,typed,element_type,counted,count,dimensions);
		if (!dimensions.empty()) throw std::runtime_error("Dimension array is not allowed on an object at byte "+std::to_string(base_t::byte_position(reader)));
	...
	static void encode_integer(int_t value,bitwise::bit_writer& writer) {
		...
		if (svalue>=-128 && svalue<=127) {
			...
			writer.write_u8('u');
			writer.write_u16(static_cast<std::uint16_t>(svalue));
		...
		} else if (svalue>=0 && svalue<=4294967295LL) {
			writer.write_u8('m');
			writer.write_u32(static_cast<std::uint32_t>(svalue));
		} else {
			writer.write_u8('L');
	...
	static void encode_value(const dom_t& node,bitwise::bit_writer& writer) {
		switch (node.type()) {
			...
			default: {
				if (base_t::is_binary(node)) {//BJData原生字节类型:[$B#<count><bytes>
...
using bjdata_t=basic_bjdata::bjdata<_Int,_Float,_Boolean,_String,_Array,_Object,_Allocator>;
using bjdata=bjdata_t<>;
