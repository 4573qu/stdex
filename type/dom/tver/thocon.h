...
//HOCON在JSON数据模型之上引入两类无法直接落入dom原生标量/容器的"未求值"节点:
//substitution(${path}/${?path})与concatenation(同一值位内多个片段的拼接/合并)。
//二者从dom_data_type续号派生;派生kind仅在本记法内部有意义,跨记法混用前需先经转换协议降级到基础模型。
...
//include资源定位方式:HIK_HEURISTIC为无包装裸串(由处理器自行启发式判断),其余对应file()/url()/classpath()包装。
...
struct hocon_sax {
	...
	//结构事件:对象/数组的开合。cnt在流式场景无法预知时传static_cast<std::size_t>(-1)。
	virtual bool start_object(std::size_t cnt)=0;
	virtual bool end_object()=0;
	virtual bool start_array(std::size_t cnt)=0;
	virtual bool end_array()=0;
	//字段起始:携带点分键路径与赋值算符(op=0对应':',1对应'=',2对应'+=');随后的片段序列以end_value收束。
	virtual bool key(path_t& path,int op)=0;
	//值即拼接序列:依次到达标量/替换片段与片段间空白(gap),或以start_object/start_array嵌套容器片段,end_value收束整值。
	...
	//include指令:spec为资源定位串,kind见hocon_include_kind,required为required(...)包裹标记。
...
//include记录:构建期按出现顺序快照(所在容器的定位路径+资源说明),求值期统一展开。
template <typename _String>
...
//SAX→DOM构建器:产出"未求值树"——替换与拼接以HDT_SUBSTITUTION/HDT_CONCATENATION派生节点悬置,
//重复键deep-merge与'+='的数组化在构建期完成;include按出现顺序连同所在容器定位一起快照,留待求值期展开。
template <typename _Hocon>
...
private:
	...
	//值帧的定位=所属容器定位+(字段路径|即将占据的数组下标),供include记录快照。
	...
	//HOCON字段落位:路径中间层强制为对象(冲突时后者胜);叶层':'/'='同为对象则深合并,
	//'+='对既有字面数组追加,否则以[value]重建(等价把+=解释为对同文件先前值的追加,规避自指替换)。
...
public:
	...
	bool end_value() override {
		...
		dom_t value;
		//收束规则:单个非unquoted片段(引号串/对象/数组/替换)即值本身;
		//单个unquoted片段或多片段保留拼接包裹,延迟到求值期分类/合并。
...
//HOCON(Human-Optimized Config Object Notation)记法:JSON超集,支持免括号根对象、换行分隔、
//键路径展开、重复键深合并、值拼接、${path}替换、include指令与'+='追加。
//解析分两阶段:SAX构建"未求值树"→resolve展开include并对替换/拼接做fixpoint求值。
...
public:
	...
	//拼接片段:node为片段值(字符串/容器/替换),lead为与前一片段之间的原文空白,unquoted标记延迟分类的裸标量。
	...
	//include处理器:库不绑定文件系统/网络/类路径的任何具体实现,资源获取与解析完全交由使用者;
	//处理器应在成功时向output写入解析后的对象并返回true,未找到返回false(required的缺失由库负责报错)。
...
public:
	...
	//---对象深合并(HOCON重复键语义:同为对象则递归合并,否则后者胜)---
...
private:
	...
	//双模式带上下文栈词法器:成员模式扫键路径/include/容器闭合,值模式扫拼接片段;
	//上下文栈记录容器种类('R'根/'O'对象/'A'数组)与"当前值是否已有片段"(concat_open),
	//以决定片段间空白发HS_WS还是作为前导空白丢弃。分隔符经emit_separator折叠:
	//抑制前导与连续换行,逗号覆写紧邻其前的换行(容忍"值\n,下一项"的宽松写法)。
	struct tokenizer {
		const char* base;
		const char* first;
		const char* last;
		...
		//容器闭合后的模式选择:回到根则等下一个成员/文档尾;
		//否则闭合的容器本身可能是拼接片段,回到值模式由scan_value据后续内容决定续拼或终止。
		...
		//include须为裸词后接空白与"资源说明"(引号串或file(/url(/classpath(/required(包装),否则退化为普通键。
		...
		//键路径:引号键或unquoted键段(内部空白保留,尾随空白剔除),'.'分层;
		//终于':'/'='/'+='或'{'(速记:合成冒号,大括号留给值扫描)。
		bool scan_keypath() {
			for (;;) {
				skip_inline();
		...
		bool scan_value() {
			...
			//此处开始一个拼接片段:若与上一片段之间存在空白,先补发HS_WS供拼接保留原文间隔。
		...
		//替换:${path}/${?path},路径段同键规则(引号段或unquoted段,内部空白保留),'.'分层,'}'收束。
		...
		//多行串:"""..."""原文保留无转义;收尾引号run多于三个时,末三个为界,其余归内容。
	...
	//SLR文法:root→object|array|object_body(免括号根);sep由词法器折叠为至多单个,
	//尾随sep由TRAILING产生式容忍;拼接为左递归part序列,片段间原文空白经HS_WS入产;
	//替换路径用独立的HS_SUBKEY以免污染字段键路径。
	...
	//归约驱动的SAX适配:shift侧发结构开启/键/间隔/include事件,归约侧发片段/替换/收束事件;
	//HP_ROOT_OBJECT/HP_ROOT_ARRAY补发end_value以收束根容器所在的隐式值帧。
...
public:
	...
	static bool sax_parse(std::string_view input,sax_t* sax) {
		...
		if (tokens.size()==1) return true;//空文档(仅EOF):按HOCON惯例视为空对象,无须进入文法。
...
protected:
	...
	//unquoted标量分类:null/true/false/整数(溢出退化浮点)/浮点/其余保持字符串。
	...
	//拼接收束:全对象→深合并(后者胜);全数组→元素顺接;容器与标量混拼报错;
	//单unquoted片段→标量分类;多标量片段→按原文间隔连接为字符串。
	...
	//fixpoint单遍:替换在目标"找到且已完全求值"时以深拷贝落位(直接自指跳过留待判环);
	//拼接先递归片段,全就绪则收束。返回是否有任何推进。
	...
	//fixpoint停滞后的清理:缺失的${path}回退环境变量(HOCON规范),仍缺失且可选则删除所在字段/元素/片段,
	//必选则报错;找到但未就绪的留待外层重入(最终仍未就绪即判环)。返回false表示该节点应被父容器移除。
	...
	//include展开:作为目标对象的底层合入(同键既有内容胜,双对象递归),
	//等价把include内容置于对象首部;目标已被非对象覆盖时按"后者胜"自然丢弃。
...
public:
	//两阶段求值入口:include展开→替换/拼接fixpoint→可选替换清理与环检测。
	...
	//已解析树的二次求值:面向手工拼装的未求值树(make_substitution/make_concatenation)。
...
protected:
	...
	static void dump_floating(string_t& out,float_t value) {
		if (!std::isfinite(static_cast<double>(value))) {
			out.append(make_string("null"));//HOCON数值继承JSON,无非有限字面量。
			return;
		}
		char buffer[64];
...
public:
	//序列化:根对象按HOCON惯例免大括号;indent<=0为紧凑单行(键值'=',成员", "),
	//indent>0为多行(键值" = ",换行分隔);ensure_ascii时非ASCII码点转\uXXXX(超BMP为代理对)。
	//未求值树(含替换/拼接)按原文形式输出,可实现无损round-trip。
	...
	//仅构建不求值:保留替换/拼接/include记录之外的原始结构,供检视或手工求值(include记录不随树保留)。
