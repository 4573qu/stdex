...
//4.不实现·DT/ER事件(Preserve.dtd):xml.h数据模型中实体在词法层展开、DOCTYPE为不透明原文,
//  树中无DTD/实体节点,EXI的结构化DT/ER在本模型中无承载对象。
...
private:
	...
	static void write_options_document(bitwise::bit_writer& writer,const exi_options& options) {
		...
		const bool need_preserve=options.preserve_comments||options.preserve_pis||options.preserve_prefixes||options.preserve_lexical_values;
		...
		if (need_lesscommon) {
			...
			if (need_preserve) {
				...
				preserve_seq.total=5;//dtd(0),prefixes(1),lexicalValues(2),comments(3),pis(4);dtd不写出(不支持DT/ER)
				...
				while (true) {
					...
					if (lesscommon_item==0) {//uncommon
						...
						while (true) {
							...
							if (preserve_item==0) decode_fail(reader,"Preserve.dtd requires DT/ER events which the xml.h data model cannot host (entities are expanded at lexing and DOCTYPE is opaque text)");
	...
	static std::size_t stc_second_count(const exi_options& options) noexcept {
		return 4+(options.preserve_prefixes?1:0)+(fidelity_group(options)?1:0);
	}
	...
	static std::size_t ec_second_count(const exi_options& options) noexcept {
		return 2+(fidelity_group(options)?1:0);
	}
	...
	static void encode_element(encode_context& ctx,const typename base_t::base_t& node,std::vector<std::vector<ns_binding>>& scopes,grammar_state& parent_state,bool parent_is_document,bool parent_in_content) {
		...
		std::size_t local_id=0;
		{
			...
			if (parent_is_document) {
				const std::size_t part1_count=parent_state.learned.size()+1+(fidelity_group(options)?1:0);
		...
		for (const auto& child:base_t::children(node)) {
			...
			if (base_t::is_element(child)) {
				...
				const std::size_t part1_count=st.learned.size()+(in_content?2:1);
				write_nbit(writer,alignment,bits_for(part1_count),st.learned.size()+(in_content?1:0));
				write_nbit(writer,alignment,bits_for(in_content?ec_second_count(options):stc_second_count(options)),in_content?2:stc_group_index(options));
	...
	//---文档编码入口---
	static binary_t encode_document(const typename base_t::base_t& root,const exi_options& options) {
		...
		while (true) {
			...
			if (ctx.frames.empty()) {
				if (!ctx.root_seen) {//DocContent
					...
					const std::size_t part1_count=st.learned.size()+1+(fidelity_group(options)?1:0);
					...
					if (code<st.learned.size()) {
						...
						ctx.root_seen=true;
						push_se(uri,local);
					} else {//二级仅CM/PI组(DT不支持):0bit,直读三级
						decoded_event event;
...
public:
	...
	binary_t encode(const exi_options& options=exi_options()) const {
		return encode_document(*this,options);
	}
	static binary_t encode(const typename base_t::base_t& root,const exi_options& options=exi_options()) {
		return encode_document(root,options);
	}
	static exi decode(const std::uint8_t* data,std::size_t size) {
		exi result;
		basic_xml::xml_sax_dom_builder<exi> builder(result,nullptr,true);
		if (!run_decode(data,size,builder)||!builder.completed()) throw std::runtime_error("Incomplete exi document");
		return result;
	}
	static exi decode(const binary_t& data) {
