...
除上述文件引用模式外，本库还提供**按文件夹引入**和**简写引入**的使用模式，该二者均服务于**项目使用模式**，在**单文件使用模式**下可能无法正常运作。

在**按文件夹引入**中，使用文件夹名，可以将文件夹内所有文件引入。如使用**`#include <bitwise/bitwise.h>`**，则可以将bitwise文件夹下的所有文件全部引入。目前支持**按文件夹引入**的目录如下：
...
- ini→type/initialization.h

同时，部分文件因不全跨平台等特定原因暂不提供对应的简写，包括的文件如下：

- structure/graph_algorithm.h
...
对于时间复杂度，F代表所采用的`func`的时间复杂度，C代表通道数，m代表$\lfloor \frac{t_{1} - t_{0}}{step} + 0.5 + 1 \rfloor$​​，k为`times`元素个数。
