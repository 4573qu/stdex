...
private:
	...
	void write_bits_lsb(std::size_t nbits,_Tp value) {
		for (std::size_t i=0;i<nbits;i++) {
			...
			curr_byte_pos_++;
			bit_pos_++;
			if (bits_in_current_byte_==CHAR_BIT) {
				buffer_.push_back(current_byte_);
				current_byte_=0;
				curr_byte_pos_=0;
			}
...
public:
	...
	explicit bit_writer(std::size_t capacity,bit_order order=is_little_endian()?BO_LSB:BO_MSB) : order_(order) {
	...
	void byte_align() {
		if (curr_byte_pos_>0) {
			if (order_==BO_MSB) current_byte_<<=(CHAR_BIT-curr_byte_pos_);
			buffer_.push_back(current_byte_);
			current_byte_=0;
			bit_pos_+=(CHAR_BIT-curr_byte_pos_);
			curr_byte_pos_=0;
		}
	}
	void flush_bits() {
	...
	void write_bits(std::size_t nbits,_Tp value) {
		...
		if (order_==BO_MSB) write_bits_msb(nbits,value);
		else write_bits_lsb(nbits,value);
	}

	void write_u8(uint8_t value) { write_bits(value,8); }
	void write_u16(uint16_t value) { write_bits(value,16); }
	void write_u32(uint32_t value) { write_bits(value,32); }
	void write_u64(uint64_t value) { write_bits(value,64); }
