...
#include <variant>
 ...
 int main(int argc, char **argv) {
   ...
   string str1 = "First string in diff";
   string str2 = "Second string in diff";
...
class diff_match_patch {
 ...
 private:
   ...
   */
 protected:
  std::list<std::variant<std::wstring, std::vector<std::wstring>>> diff_linesToChars(const std::wstring &text1, const std::wstring &text2); // return elems 0 and 1 are QString, elem 2 is QStringList
   ...
   */
 private:
  std::wstring diff_linesToCharsMunge(const std::wstring &text, std::vector<std::wstring> &lineArray,
   ...
   */
 private:
  void diff_charsToLines(std::list<Diff> &diffs, const std::vector<std::wstring> &lineArray);
 ...
 protected:
   ...
   */
 protected:
  std::vector<std::wstring> diff_halfMatch(const std::wstring &text1, const std::wstring &text2);
   ...
   */
 private:
  std::vector<std::wstring> diff_halfMatchI(const std::wstring &longtext, const std::wstring &shorttext, int i);
 ...
 private:
  static inline std::wstring safeMid(const std::wstring &str, int pos) {
    return (pos == str.length()) ? L"" : str.substr(pos);
  }
 ...
 private:
  static inline std::wstring safeMid(const std::wstring &str, int pos, int len) {
    return (pos == str.length()) ? L"" : str.substr(pos, len);
  }
};
